defmodule ExL7.QueryParserTest do
  ...
  describe "field selectors" do
    ...
    test "field with spaces" do
      {:ok, query} = parse("PV1|18(2,MR FIN)")
      assert query.all_segments == false
    ...
    test "component with spaces" do
      {:ok, query} = parse("PID|10(0,MR OTHER SPACE)^8")
      assert query.all_segments == false
      assert query.segment == "PID"
  ...
  describe "invalid" do
    setup do
      {:ok, invalid: {:error, "Invalid Query"}}
    end

    test "empty", context do
      assert parse("") == context[:invalid]
    end

    test "segment only", context do
      assert parse("PID") == context[:invalid]
    end

    test "invalid segment", context do
      assert parse("PID3|3") == context[:invalid]
    end

    test "invalid field", context do
      assert parse("PID|a") == context[:invalid]
    end

    test "multi multi", context do
      assert parse("PId[4][4][5]") == context[:invalid]
    end
  end
end
